...
const Friend = ({ friendId, name, subtitle, userPicturePath }) => {  // Destructure friendId, name, subtitle, userPicturePath
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { _id } = useSelector((state) => state.user);
    const token = useSelector((state) => state.token);
    const friends = useSelector((state) => state.user.friends);  // Get friends from array attached to the user
    ...
    return (
        ...
        <FlexBetween gap="1rem">
            ...
            <Box
            onClick={() => {
                navigate(`/profile/${friendId}`);  // Navigate to friend's profile page
                navigate(0);  // Reset the navigation stack
            }}
            >
            <Typography
                color={main}
